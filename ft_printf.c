...
/*   Updated: 2023/01/08 20:12:25 by lda-cunh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_flag(char c, va_list list, int *counter)
{
	if (c == 'c')
		...
		*counter += ft_putstr(va_arg(list, char *));
		...
		ft_pointer_adress(va_arg(list, unsigned long int), \
			"0123456789abcdef", counter);
	else if (c == 'd' || c == 'i')
		ft_putnbr(va_arg(list, int), counter);
	else if (c == 'u')
		ft_putnbr_unsigned(va_arg(list, unsigned int), "0123456789", counter);
	else if (c == 'x')
		ft_hexa(va_arg(list, unsigned int), "0123456789abcdef", counter);
	else if (c == 'X')
		ft_hexa(va_arg(list, unsigned int), "0123456789ABCDEF", counter);
	else if (c == '%')
